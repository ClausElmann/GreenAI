  ...
  sprint_2:
    ...
    items:
      ...
      - file: docs/SSOT/governance/ssot-update-protocol.md
        ...
        red_threads: []
        content_must_include:
          - triggers: new pattern used → update SSOT in same PR
          - triggers: new error code added → update ResultExtensions + result-pattern.md
          - triggers: new red thread discovered → update RED_THREAD_REGISTRY.md
          - forbidden: merging code that introduces undocumented pattern
      ...
      - file: docs/SSOT/governance/ai-boundaries.md
        ...
        red_threads: []
        content_must_include:
          - ALLOWED: all code changes, file creation, test execution, build
          - FORBIDDEN: git push/commit/reset/rebase (prepare only, wait for user)
          - FORBIDDEN: delete source files without confirmation
          - FORBIDDEN: prod DB mutating queries without confirmation
          - FORBIDDEN: architecture changes without SSOT update
          - STOP_AND_ASK: destructive actions, unclear requirements
